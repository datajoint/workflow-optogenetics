...
#
# The major component of the current workflow is Element Optogenetics (see [github](https://github.com/datajoint/element-optogenetics) and [documentation](https://datajoint.com/docs/elements/element-optogenetics)). Many Elements require configurations in the field `custom` in `dj.config`:
#
# ### Database prefix
#
# Giving a prefix to schema could help on the configuration of privilege settings. For example, if we set prefix `neuro_`, every schema created with the current workflow will start with `neuro_`, e.g. `neuro_lab`, `neuro_subject`, etc.
#
...
#
# For first-time users, we recommend saving globally. This will create a hidden configuration file saved in your root directory, loaded whenever there is no local version to override it.
#
